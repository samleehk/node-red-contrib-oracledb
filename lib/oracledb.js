    ...
    function OracleServer(n) {
        ...
        node.query = function (requestingNode, query, values, resultAction, resultSetLimit, msg) {
            ...
            if (node.connection) {
                ...
                var processingLog = setInterval(() => {
                    requestingNode.log(`Oracle long process [10 seconds] for ${msg._msgid}`);
                    requestingNode.log(`Query is ${query}`);
                }, 10000);
                var options = {
                    autoCommit: true,
